<?php
/**
 * Pandapay Payment Request API
 *
 * @package WooCommerce_Pandapay/Classes/Payment_Request
 * @since   3.1.0
 * @version 3.1.0
 */
...
/**
 * WC_Pandapay_Payment_Request class.
 */
class WC_Pandapay_Payment_Request {
	...
	public function __construct() {
		...
		add_action( 'wc_ajax_wc_pandapay_get_cart_details', array( $this, 'ajax_get_cart_details' ) );
		add_action( 'wc_ajax_wc_pandapay_get_shipping_options', array( $this, 'ajax_get_shipping_options' ) );
		add_action( 'wc_ajax_wc_pandapay_update_shipping_method', array( $this, 'ajax_update_shipping_method' ) );
		add_action( 'wc_ajax_wc_pandapay_create_order', array( $this, 'ajax_create_order' ) );
	}

	/**
	 * Check if Pandapay gateway is enabled.
	 *
	 * @return bool
	 */
	protected function is_activated() {
		$options             = get_option( 'woocommerce_pandapay_settings', array() );
		$enabled             = isset( $options['enabled'] ) && 'yes' === $options['enabled'];
		$pandapay_checkout     = isset( $options['pandapay_checkout'] ) && 'yes' !== $options['pandapay_checkout'];
		$request_payment_api = isset( $options['request_payment_api'] ) && 'yes' === $options['request_payment_api'];

		return $enabled && $pandapay_checkout && $request_payment_api && is_ssl();
	}
	...
	public function scripts() {
		...
		wp_localize_script(
			...
			'wcPandapayPaymentRequestParams',
			array(
				'ajax_url' => WC_AJAX::get_endpoint( '%%endpoint%%' ),
				'pandapay'   => array(
					'key'                => $this->get_publishable_key(),
					'allow_prepaid_card' => apply_filters( 'wc_pandapay_allow_prepaid_card', true ) ? 'yes' : 'no',
				),
				'nonce'    => array(
					'payment'         => wp_create_nonce( 'wc-pandapay-payment-request' ),
					'shipping'        => wp_create_nonce( 'wc-pandapay-payment-request-shipping' ),
					'update_shipping' => wp_create_nonce( 'wc-pandapay-update-shipping-method' ),
					'checkout'        => wp_create_nonce( 'woocommerce-process_checkout' ),
				),
				'i18n'     => array(
					'no_prepaid_card'  => __( 'Sorry, we\'re not accepting prepaid cards at this time.', 'woocommerce-gateway-pandapay' ),
					/* translators: Do not translate the [option] placeholder */
					'unknown_shipping' => __( 'Unknown shipping option "[option]".', 'woocommerce-gateway-pandapay' ),
				),
			)
		);
	}
	...
	public function ajax_get_cart_details() {
		check_ajax_referer( 'wc-pandapay-payment-request', 'security' );
		...
		$data = array(
			...
			'order_data'        => array(
				'total' => array(
					'label'  => __( 'Total', 'woocommerce-gateway-pandapay' ),
					'amount' => array(
	...
	public function ajax_get_shipping_options() {
		check_ajax_referer( 'wc-pandapay-payment-request-shipping', 'security' );
	...
	public function ajax_update_shipping_method() {
		check_ajax_referer( 'wc-pandapay-update-shipping-method', 'security' );
	...
	public function ajax_create_order() {
		if ( WC()->cart->is_empty() ) {
			wp_send_json_error( __( 'Empty cart', 'woocommerce-gateway-pandapay' ) );
		}
	...
	protected function compute_display_items( $shipping_method_id ) {
		...
		$items = array(
			...
			array(
				'label' => __( 'Subtotal', 'woocommerce-gateway-pandapay' ),
				'amount' => array(
		...
		if ( 0 < $tax_total ) {
			$items[] = array(
				'label'  => __( 'Tax', 'woocommerce-gateway-pandapay' ),
				'amount' => array(
					'currency' => $currency,
					'value'    => $tax_total,
				),
...
new WC_Pandapay_Payment_Request();
